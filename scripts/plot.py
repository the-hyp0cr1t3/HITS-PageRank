...
for idx, iters in enumerate(iters_ls):
	...
	ax4.plot(df['n'], df['hits run_time (ms)'] ** (1/3), label=f'{iters} iterations')

ax1.text(x=30, y=3400, fontsize='12', color='grey', s='$\\mathcal{O}(n^3 \\cdot iterations)$ or $\\mathcal{O}(n^3 \\cdot \\mathrm{log}_2(iterations))$')
...
ax3.text(x=30, y=210, fontsize='12', color='grey', s='$\\mathcal{O}(n^3 \\cdot iterations)$ or $\\mathcal{O}(n^3 \\cdot \\mathrm{log}_2(iterations))$')
ax3.set_title('HITS execution time vs n', fontsize='15')
ax3.legend(loc="upper left")
ax3.set_xlabel('n')
ax3.set_ylabel('HITS execution time (ms)')

ax4.set_title('Cube root of HITS execution time vs n\nshowing approximately linear relationship', fontsize='15', wrap=True)
ax4.legend(loc="upper left")
ax4.set_xlabel('n')
ax4.set_ylabel('Cube root of HITS execution time (ms)')
